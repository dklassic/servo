...
impl InlineFormattingContextLayout<'_> {
    ...
    pub(super) fn push_glyph_store_to_unbreakable_segment(
        ...
        let insertion_point = if let Some(insertion_point_index) = text_run.insertion_point {
            if (range.contains(insertion_point_index) || insertion_point_index == range.end()) &&
                (range.begin() != insertion_point_index || range.begin().0 == 0)
            {
                Some(insertion_point_index - range.begin())
            } else {
                None
            }
        } else {
